...
Function New-RestartJobBG {
    ...
    $Action = New-ScheduledTaskAction -Execute 'powershell.exe' -Argument "$currentdir\ssm.ps1 restart $server" -WorkingDirectory "$currentdir"
    ...
    Register-ScheduledTask -TaskName "$server AutoRestart" -InputObject $Task -User "$UserName" -Password "$Password" -ea SilentlyContinue
}
Function New-RestartJob {
    ...
    $Task = New-ScheduledTask -Action $Action -Trigger $Trigger -Settings $Settings
    Write-Host "Creating Task........" -F M -B Black
    Register-ScheduledTask -TaskName "$server AutoRestart" -InputObject $Task
}