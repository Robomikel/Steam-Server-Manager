...
Function Get-ValidateServer {
    ...
    If ($anon -eq "yes") {
        .\steamCMD +@ShutdownOnFailedCommand 1 +@NoPromptForPassword 1 +login anonymous +force_install_dir $serverdir +app_update $APPID $Branch validate +Exit
    }
    Else {
        .\steamCMD +@ShutdownOnFailedCommand 1 +login $username +force_install_dir $serverdir +app_update $APPID $Branch validate +Exit
    }
    If ( !$? ) {
