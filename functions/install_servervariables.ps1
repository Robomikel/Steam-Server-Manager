...
Function New-CreateVariables {
    ...
    New-Item $currentdir\$serverfiles\Variables-$serverfiles.ps1 -Force | Out-File -Append -Encoding Default  $ssmlog
    ...
    If ($servercfg) {
        Add-Content  $currentdir\$serverfiles\Variables-$serverfiles.ps1  "#                            Game-Server-Config"
        Add-Content  $currentdir\$serverfiles\Variables-$serverfiles.ps1  "`$global:servercfg           = `"$global:servercfg`""
    }
    If ($servercfgdir) {
    ...
    If ($beaconport) {
        Add-Content  $currentdir\$serverfiles\Variables-$serverfiles.ps1  "#                            server Beacon port"
        Add-Content  $currentdir\$serverfiles\Variables-$serverfiles.ps1  "`$global:beaconport          = `"$global:beaconport`""
    }
    If ($peerport) {
        Add-Content  $currentdir\$serverfiles\Variables-$serverfiles.ps1  "#                            Peer Port "
        Add-Content  $currentdir\$serverfiles\Variables-$serverfiles.ps1  "`$global:peerport            = `"$global:peerport`""
    }
    If ($sourcetvport) {
    ...
    If ($lighting) {
        Add-Content  $currentdir\$serverfiles\Variables-$serverfiles.ps1  "#                            Map Lighting"
        Add-Content  $currentdir\$serverfiles\Variables-$serverfiles.ps1  "`$global:lighting           = `"$global:lighting`""
    }
    If (($modsenabled) -and (($modsenabled -ne $false))) {
        Add-Content  $currentdir\$serverfiles\Variables-$serverfiles.ps1  "#                            Mods enabled"
        Add-Content  $currentdir\$serverfiles\Variables-$serverfiles.ps1  "`$global:modsenabled           = `"$global:modsenabled`""
    }
    If ($oauthtoken) {
        Add-Content  $currentdir\$serverfiles\Variables-$serverfiles.ps1  "#                            Mods OAuth"
        Add-Content  $currentdir\$serverfiles\Variables-$serverfiles.ps1  "`$global:oauthtoken           = `"$global:oauthtoken`""
    }
    If ($defaultmap) {
    ...
    If ($numplayers) {
        Add-Content  $currentdir\$serverfiles\Variables-$serverfiles.ps1  "#                            number of Players  "
        Add-Content  $currentdir\$serverfiles\Variables-$serverfiles.ps1  "`$global:numplayers          = `"$global:numplayers`""
    }
    If ($coopplayers) {
    ...
    If ($logdirectory) {
        Add-Content  $currentdir\$serverfiles\Variables-$serverfiles.ps1  "#                            log directory   "
        Add-Content  $currentdir\$serverfiles\Variables-$serverfiles.ps1  "`$global:logdirectory         = `"$global:logdirectory`""
    }
    If ($consolelog) {
        Add-Content  $currentdir\$serverfiles\Variables-$serverfiles.ps1  "#                            consolelog directory   "
        Add-Content  $currentdir\$serverfiles\Variables-$serverfiles.ps1  "`$global:consolelog         = `"$global:consolelog`""
    }
    If ($launchParams) {
