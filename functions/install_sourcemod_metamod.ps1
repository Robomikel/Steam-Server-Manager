...
Function Get-SourceMetaMod {
    ...
    Copy-Item  $currentdir\metamod\* -Destination $currentdir\$server\$MODDIR -Force -Recurse
    If (!$?) { 
    ...
    Copy-Item  $currentdir\sourcemod\* -Destination $currentdir\$server\$MODDIR -Force -Recurse
    If (!$?) { 
